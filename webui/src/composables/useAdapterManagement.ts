...
import type { AdapterDraftRow, CoreTranslate } from '@/utils/adapterStatus'
...
export function useAdapterManagement(options: {
  ...
  const normalizedDraft = computed(() =>
    normalizeAdapterModules(draftRows.value.map(item => item.value)),
  )
  const hasPendingChanges = computed(() =>
    JSON.stringify(normalizedDraft.value) !== JSON.stringify(savedModules.value),
  )
  ...
  const normalizationNotes = computed(() => {
    ...
    if (duplicateModules.value.length > 0) {
      notes.push(options.t('core.adapterDuplicateRows', {
        modules: duplicateModules.value.join(', '),
      }))
    }
