...
} from '@/composables/chatComposer'
import { formatBytes } from '@/composables/useChatMedia'
...
export function useChatComposer(options: {
  ...
  function removeComposerImage(id: string) {
    ...
    composer.querySelector<HTMLElement>(`[data-image-id="${id}"]`)?.remove()
  ...
  function removeComposerMention(id: string) {
    ...
    composer.querySelector<HTMLElement>(
      `[data-kind="mention-token"][data-mention-id="${id}"]`,
    )?.remove()
