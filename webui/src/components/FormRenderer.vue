...
function addMapEntry(field: MapField) {
  const obj = {
    ...((props.modelValue[field.key] as Record<string, unknown>) || {}),
  };
  const tempKey = `__temp_${Date.now()}`;
  obj[tempKey] =
    field.value_schema?.kind === "primitive"
    ...
    <template v-for="field in sortedFields" :key="field.key">
      ...
      >
        <div class="flex flex-wrap items-center gap-2">
          <Label
            ...
            class="font-medium"
            :class="{
              'cursor-pointer': (field as PrimitiveField).type === 'bool',
            }"
          >
            {{ (field as PrimitiveField).label }}
          </Label>
          <span
            v-if="(field as PrimitiveField).required"
            class="text-destructive text-xs"
            >*</span
          >
          <span
            class="rounded-sm bg-accent px-1.5 py-0.5 text-xs font-medium text-accent-foreground"
          >
        ...
        <div>
          ...
          <Input
            v-else-if="
              (field as PrimitiveField).type === 'int' ||
              (field as PrimitiveField).type === 'float'
            "
            :id="field.key"
            type="number"
            :model-value="
              modelValue[field.key] !== undefined
      ...
      >
        ...
        <div class="border-l-2 border-border/70 pl-4">
          <FormRenderer
            ...
            :model-value="
              (modelValue[field.key] as Record<string, unknown>) || {}
            "
            @update:model-value="
              (v: Record<string, unknown>) => updateField(field.key, v)
            "
          />
        </div>
      </div>
      ...
      >
          ...
          >
            <div class="flex justify-between items-center">
              ...
              <Button
                ...
                size="icon"
                aria-label="删除此项"
                @click="removeArrayItem(field as ArrayField, idx)"
              >
            ...
            <FormRenderer
              ...
              @update:model-value="
                (v: Record<string, unknown>) =>
                  updateArrayItem(field as ArrayField, idx, v)
              "
            />
            <Input
              v-else
              ...
              @update:model-value="
                (v: string | number) =>
                  updateArrayItem(field as ArrayField, idx, String(v))
              "
            />
          </div>
        </div>
        <Button
          variant="outline"
          size="sm"
          @click="addArrayItem(field as ArrayField)"
        >
      ...
      >
        ...
        <div class="divide-y divide-border border-l-2 border-border/70 pl-4">
          <div
            v-for="(v, k) in (modelValue[field.key] as Record<
              string,
              unknown
            >) || {}"
            :key="k"
          ...
          >
            <Input
              ...
              @update:model-value="
                (nk: string | number) =>
                  updateMapKey(field as MapField, String(k), String(nk))
              "
            />
            ...
            <Input
              ...
              @update:model-value="
                (val: string | number) =>
                  updateMapValue(field as MapField, String(k), String(val))
              "
            />
            <FormRenderer
              ...
              @update:model-value="
                (val: Record<string, unknown>) =>
                  updateMapValue(field as MapField, String(k), val)
              "
            />
            <Button
              variant="ghost"
              size="icon"
              aria-label="删除此项"
              @click="removeMapEntry(field as MapField, String(k))"
            >
              <X class="size-4" aria-hidden="true" />
            </Button>
          </div>
        ...
        <Button
          variant="outline"
          size="sm"
          @click="addMapEntry(field as MapField)"
        >
          <Plus class="size-4" /> 添加
        </Button>
      </div>
