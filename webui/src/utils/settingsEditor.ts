...
export function displayChoiceTitle(choice: SettingChoice) {
...
export function buildSchemaDefaultValue(schema: SettingSchema | null): unknown {
  if (!schema) {
    return ''
  }
...
export function buildSettingsForm(fields: SettingField[]) {
...
export function isSequenceChipField(field: SettingField) {
...
export function isNullableBoolField(field: SettingField) {
  return field.type === 'bool' && field.allows_null
}

export function textInputType(field: SettingField) {
  if (field.secret) {
    return 'password'
  }
  return 'text'
...
function normalizeSequenceValue(field: SettingField, rawValue: unknown) {
...
function normalizeBySchema(schema: SettingSchema, rawValue: unknown): unknown {
  if (rawValue == null) {
    if (schema.allows_null) {
      return null
    }
  ...
  if (schema.choices.length > 0) {
    ...
    if (!schema.choices.some(choice =>
      JSON.stringify(choice.value) === JSON.stringify(normalized),
    )) {
...
function normalizeScalarNumberValue(typeName: 'int' | 'float', rawValue: unknown) {
  if (rawValue === null) {
    return null
  }
...
function normalizeComparableFieldValue(field: SettingField, value: unknown) {
  if (value == null) {
    return null
  }
  if (field.schema) {
...
function comparableBySchema(schema: SettingSchema, value: unknown): unknown {
  if (value == null) {
    return null
  }
  if (schema.type === 'int' || schema.type === 'float') {
  ...
  if ((schema.type === 'dict' || schema.fields.length > 0) && isPlainObject(value)) {
    ...
    if (schema.type === 'dict') {
      ...
      return Object.fromEntries(
        entries.map(([key, item]) => [
          key,
          valueSchema ? comparableBySchema(valueSchema, item) : item,
        ]),
      )
    }
    ...
    return Object.fromEntries(
      entries.map(([key, item]) => [
        key,
        fieldMap.has(key) ? comparableBySchema(fieldMap.get(key)!, item) : item,
      ]),
    )
  }
  return value
...
function resolveNullableFieldValue(field: SettingField, value: unknown) {
