...
export function getTextContent(segments: ChatSegment[], t: ChatTranslate) {
  return segments
    .map(segment => {
...
export function summarizeReplyMessage(message: MessageReceivePayload, t: ChatTranslate) {
...
export function formatSessionTitle(item: SessionListItem, t: ChatTranslate) {
  ...
  return t('chat.sessionLabel', { id: sessionId.slice(-4) })
}
