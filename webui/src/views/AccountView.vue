...
function onSubmit() {
  ...
  mutate(
    { old_password: form.oldPassword, new_password: form.newPassword },
    {
      ...
      onError: (e: Error) => toast.error(e.message || t('account.changeFailed')),
    },
  )
}
</script>
      ...
      <CardContent>
        <div v-if="isError" class="mb-4 rounded-lg border border-destructive/50 bg-destructive/10 p-4">
          <div class="flex items-center gap-2">
            ...
            <p class="text-sm font-medium text-destructive">{{ $t('account.operationFailed') }}</p>
          </div>
          <p class="mt-1 text-sm text-destructive/80">{{ (error as Error)?.message }}</p>
          <Button variant="outline" size="sm" class="mt-2" @click="() => reset()">{{ $t('error.retry') }}</Button>
        </div>
