...
def test_ai_plugin_registers_future_task_recovery_hook() -> None:
    project_root = Path(__file__).resolve().parents[2]
    plugin_source = (project_root / "apeiria" / "builtin_plugins" / "ai.py").read_text(
        encoding="utf-8"
    )

    assert "recover_scheduled_tasks" in plugin_source
    assert "on_startup(_recover_ai_future_tasks)" in plugin_source
