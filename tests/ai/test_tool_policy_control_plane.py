...
def test_operations_list_capabilities_uses_lifecycle_inventory(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    record = AICapabilityInventoryRecord(
        name="memory.query",
        kind="executable",
        origin="builtin",
        ...
        binding_type="local_tool",
        availability="ready",
    )

    def fake_ensure(**kwargs: object) -> AILifecycleSnapshot:
        assert kwargs == {"source": "admin_fallback"}
        return AILifecycleSnapshot(
            initialized=True,
            initialization_source="admin_fallback",
            components=(),
            capabilities=(record,),
        )
    ...
    assert _Operations().list_capabilities() == [record]


def test_webui_capability_schema_projects_unified_contract_view() -> None:
    from apeiria.app.ai.lifecycle import AICapabilityInventoryRecord
    from apeiria.webui.routes.ai.tools_schemas import to_ai_capability_item

    item = to_ai_capability_item(
        AICapabilityInventoryRecord(
            name="plugin.partial",
            kind="executable",
            origin="plugin",
            description="Partial host action.",
            input_schema={"type": "object"},
            read_only=False,
            ...
            diagnostics=("missing capability contract",),
        )
    )

    assert item.capability_name == "plugin.partial"
    assert item.kind == "executable"
    assert item.origin == "plugin"
    assert item.availability == "incomplete"
    assert item.diagnostics == ["missing capability contract"]
    assert item.risk_label == "高风险"
