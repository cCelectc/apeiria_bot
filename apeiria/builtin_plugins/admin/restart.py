"""Owner-facing restart command with post-restart notification."""
...
async def handle_restart(bot: Bot, event: Event) -> None:
    ...
    await _restart.send(t("admin.restart.scheduled"))
...
async def _on_reconnect(bot: Bot) -> None:
    ...
    logger.warning("Restart notification failed after {} attempts", _MAX_RETRIES)
