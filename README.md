...
- Apeiria 提供项目组织、配置管理、项目运行时管理和可视化运维能力
- NoneBot 仍然是事件内核；Apeiria 在其之上补充显式的项目运行时层
...
- 内置插件：`apeiria.builtin_plugins.ai`、`apeiria.builtin_plugins.admin`、`apeiria.builtin_plugins.help`、`apeiria.builtin_plugins.render`、`apeiria.builtin_plugins.web_ui`
- Web UI：仪表盘、核心配置编辑、插件启停与配置编辑、插件商店、权限管理、日志、Web Chat、账户管理
- 宿主机 CLI：环境初始化、修复、导入导出、健康检查，驱动、适配器、插件管理，Web UI 账户恢复与注册码管理

## 运行时分层

- NoneBot 仍然负责事件循环、驱动、适配器和插件执行，是 Apeiria 的事件内核
- `apeiria/runtime/*` 负责项目层运行时契约，把启动编排、运行时上下文、入口归一化和管理面控制面显式化
- 当前启动主线围绕 `ApeiriaBootstrapper` 组织，并拆分为 environment、config、user extensions、framework、runtime、user plugins 等阶段
- runtime 阶段会构造并注册 `ApeiriaRuntime`，把项目级服务句柄集中到统一运行时上下文
- `ApeiriaEntry` 用于归一化顶层入口语义，当前已用于 AI trace 等运行时上下文场景
- `ApeiriaControlPlane` 提供面向管理面的运行时入口，当前已接入部分 Web UI 读取路径
...
.
├── apeiria/
│   ├── bootstrap.py              # 兼容入口：转入 runtime/bootstrap 启动流程
│   ├── runtime/                  # 项目运行时层：启动阶段、运行时上下文、入口与控制面
│   ├── bot/                      # Bot 入口与系统级钩子
...
Apeiria 现在通过显式的项目运行时层与 NoneBot 协作，而不再只依赖 `bootstrap.py`、`_framework_loader.py`、`_user_loader.py` 这组历史入口的口头约定。

当前启动流程如下：

1. `ApeiriaBootstrapper` 先执行 environment phase，整理项目环境与扩展环境。
2. config phase 读取 `apeiria.config.toml` 与环境变量，生成 `nonebot.init(...)` 所需参数。
3. user extensions phase 执行 `user_bot.py` 中的本地启动钩子，并注册项目适配器。
4. framework phase 完成 NoneBot 初始化、框架依赖插件加载和 Apeiria 内置插件加载。
5. runtime phase 构造 `ApeiriaRuntime`、附加 `ApeiriaControlPlane`，并把当前运行时注册到全局上下文。
6. user plugins phase 再从 `apeiria.plugins.toml` 加载用户插件。
...
- `bootstrap.py` 仍保留兼容入口，但新的项目级启动编排应优先放入 `apeiria/runtime/*`
