...
function resolveMissingModelAction (
  input: AISetupWorkflowInput,
): AISetupNextAction {
...
export function deriveAISetupWorkflow (
  input: AISetupWorkflowInput,
): AISetupWorkflow {
  ...
  if (!hasProvider) {
    ...
    steps = [
      ...
      createStep(
        'discovery',
        hasFetchedModels ? 'complete' : 'current',
        nextAction.kind,
      ),
      createStep(
        'model',
        hasFetchedModels ? 'current' : 'pending',
        nextAction.kind,
      ),
      createStep('defaultModel', 'pending'),
      createStep('profile', needsProfile ? 'pending' : 'complete'),
